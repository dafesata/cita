class Evolucion < ApplicationRecord
	...
	belongs_to :historia, :foreign_key => "historia_id" 
	...
	validate :historia_exist
	validates :paciente_id, :especialidad_id, :profesional_id, :peso, :talla, :tensionarterial, :fecha, :descripcion, :idiagnostica1, presence: true

	def historia_exist		
		 return errors.add(:paciente_id, ' no tiene Historia') if (!Historium.exists?(paciente_id: self.paciente_id))
	end
