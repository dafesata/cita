class Profesional < ActiveRecord::Base
	...
	before_destroy :destroy_citas, :destroy_historia, :destroy_evolucion
	...
	def destroy_citas
   	...
   	def destroy_historia
     	Historia.where(:profesional_id => self.numero_id ).destroy_all  
   	end

   	def destroy_evolucion
   		Evolucion.where(:profesional_id => self.numero_id ).destroy_all 
   	end
end
